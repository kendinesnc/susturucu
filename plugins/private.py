...
LEARN_TEXT = (
    "u bot yalnızca satır içi modda çalışır, örnek bir kullanım şöyle olur "
...
DEFAULT_TEXT = (
    ...
    "yalnızca satır içi modda çalışır\n\n"
    "[Kanal](https://t.me/netdbots)"
    " | [Developer](https://t.me/Azerbesk)"
)
DEFAULT_REPLY_MARKUP = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(
                "Denenecek Bir Sohbet Seçin",
                switch_inline_query=""
            ),
            InlineKeyboardButton(
                "Bu Sohbette Deneyin",
...
async def show_main_page(_, cq: CallbackQuery):
    ...
    if cq.data == "learn_next":
        await cq.answer("Artık satır içi moda dönebilirsiniz")
...
async def list_whispers(_, cq: CallbackQuery):
    ...
    if len(user_whispers) == 0:
        text = "Hiç fısıltı yok"
    else:
        text = f"Fısıltın Var **{len(user_whispers)}**"
...
async def delete_my_whispers(_, cq: CallbackQuery):
    ...
    if len(deleted_whispers) == 0:
        await cq.answer("hiç fısıltı yok")
    else:
        await cq.answer(f"{len(deleted_whispers)} Fısıltı Kaldırıldı.")
